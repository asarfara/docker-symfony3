...
if [ -z "$TRAVIS_PULL_REQUEST" ] || [ "$TRAVIS_PULL_REQUEST" == "false" ]; then
  ...
  if [ "$TRAVIS_BRANCH" == "develop" ]; then
    ...
    $TRAVIS_BUILD_DIR/deploy/ecs-deploy.sh -c $TASK_DEFINITION -n $SERVICE -i $REMOTE_IMAGE_URL/$PHP_IMAGE_NAME:latest
    $TRAVIS_BUILD_DIR/deploy/ecs-deploy.sh -c $TASK_DEFINITION -n $SERVICE -i $REMOTE_IMAGE_URL/$WEB_IMAGE_NAME:latest
  else
